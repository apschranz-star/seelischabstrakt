...
export function ProductCard({ product: source }: { product: Product }) {
  ...
  const basePrice = formatBasePrice(product, activeRegion, lang);
  ...
  return (
    ...
    >
      ...
      <Link
        ...
        aria-label={`${product.name}, ${t({ de: "Produktdetails", en: "product details" })}`}
      >
      ...
      <div className="mt-auto pt-4">
        <div className="flex flex-col gap-0.5 border-t border-line pt-3">
          <p className="font-mono text-[15px] tabular-nums text-ink">
            {formatForRegion(product.priceCents, activeRegion, lang)}
          </p>
          {basePrice ? (
