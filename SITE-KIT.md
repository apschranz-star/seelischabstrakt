...
4. `netlify.toml` bleibt wie sie ist. Für GitHub Pages den Workflow `.github/workflows/schranz-ai-pages.yml` kopieren, `schranz-ai` durch den neuen Ordnernamen ersetzen, und in `farida-pages.yml` den Ordner in die Zeile mit `! -name jing ! -name schranz-ai ! -name portfolio ! -name shop` aufnehmen.
